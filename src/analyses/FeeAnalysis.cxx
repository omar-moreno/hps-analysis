...
void FeeAnalysis::initialize() { 
    this->bookHistograms(); 
    matcher->enablePlots(true);
}

void FeeAnalysis::processEvent(HpsEvent* event) { 
...
void FeeAnalysis::finalize() { 

    std::cout << "//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%//" << std::endl;  
    std::cout << "[ FeeAnalysis ] Total number of events: " << event_counter << std::endl;
    std::cout << "[ FeeAnalysis ] Total number of single1 triggers: " 
              << single1_trigger_counter << std::endl;
    /*std::cout << "[ FeeAnalysis ] Total number of single1 trigger events with SVT "
              << "bias ON: " << bias_on_counter << std::endl; 
    std::cout << "[ FeeAnalysis ] Total number of single1 trigger events with SVT "
              << "bias ON and in closed position: " << svt_closed_position_counter << std::endl; */
    std::cout << "[ FeeAnalysis ] Total number of events with tracks: "
...
void FeeAnalysis::bookHistograms() { 
...
std::string FeeAnalysis::toString() { 
    std::string string_buffer = "Class Name: " + class_name; 
    return string_buffer;
}


bool FeeAnalysis::passEnergyCut(EcalCluster* cluster) { 
...
bool FeeAnalysis::passClusterTimeCut(EcalCluster* cluster) {   
    if (cluster->getClusterTime() < 42 || cluster->getClusterTime() > 49.5) return false;

    return true;   
}

bool FeeAnalysis::passClusterSizeCut(EcalCluster* cluster) { 
    if (cluster->getEcalHits()->GetEntriesFast() < 3) return false;
    return true;
}

bool FeeAnalysis::isEdgeCrystal(EcalHit* hit) { 
...
bool FeeAnalysis::isMatch(EcalCluster* cluster, SvtTrack* track) { 
